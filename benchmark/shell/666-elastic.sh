...
bullet_flag='false'
ode_flag='true'
mujoco_flag='false'     # mujoco cannot simulate elastic collision
dart_flag='false'
...
do
    ...
    if [ "$raisim_flag" == 'true' ]; then
        if [ "$RAISIM_ON" == "ON" ]; then
            ...
            do
                timeout 600 ../sim/raiSim/benchmark/Rai666Benchmark --nogui --erp-on=$erpon --dt=$dt --log --elastic
            done
        else
    ...
    if [ "$bullet_flag" == 'true' ]; then
        if [ "$BTSIM_ON" == "ON" ]; then
            ...
            do
                for erpon in true false
                do
                    timeout 600 ../sim/bulletSim/benchmark/Bt666Benchmark --nogui --erp-on=$erpon --dt=$dt --solver=$solver --log --elastic
                done
            done
        else
    ...
    if [ "$ode_flag" == 'true' ]; then
        if [ "$ODESIM_ON" == "ON" ] ; then
            ...
            do
                for erpon in true false
                do
                    timeout 600 ../sim/odeSim/benchmark/Ode666Benchmark --nogui --erp-on=$erpon --dt=$dt --solver=$solver --log --elastic
                done
            done
        else
    ...
    if [ "$mujoco_flag" == 'true' ]; then
        if [ "$MJCSIM_ON" == "ON" ] ; then
            ...
            do
                ...
                timeout 600 ../sim/mujocoSim/benchmark/Mjc666Benchmark --nogui --dt=$dt --solver=$solver --log --elastic
            done
        else
    ...
    if [ "$dart_flag" == 'true' ]; then
        if [ "$DARTSIM_ON" == "ON" ] ; then
            ...
            do
                ...
                timeout 600 ../sim/dartSim/benchmark/Dart666Benchmark --nogui --dt=$dt --solver=$solver --log --elastic
            done
        else
