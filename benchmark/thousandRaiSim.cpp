...
int main(int argc, char* argv[]) {
  ...
  if (is_visualization_mode)
    raiSim = new rai_sim::World_RG(800,600,0.6);

  // objects
  auto checkerboard = raiSim->addCheckerboard(5.0, 200.0, 200.0, 0.1);
      ...
      for(int k = 0; k < nPerDim; k++) {
        ...
        switch(shape) {
          case 0:
            ...
            obj = raiSim->addSphere(0.5, 1500.0);
            break;
          case 1:
            // box (aluminum density)
            obj = raiSim->addBox(0.8, 0.4, 0.2, 170.0);
            break;
          case 2:
            // capsule (aluminum density)
            obj = raiSim->addCapsule(0.2, 0.6, 300.0);
            break;
        }
  ...
  if(is_visualization_mode) {
    raiSim->cameraFollowObject(objectPtrList.at(nPerDim*nPerDim*nPerDim/2), {0, 30.0, 10.0});
