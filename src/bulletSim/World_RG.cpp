...
bullet_sim::SingleBodyHandle bullet_sim::World_RG::addBox(double xLength,
                                                          double yLength,
                                                          double zLength,
                                                          double mass,
...
bullet_sim::SingleBodyHandle bullet_sim::World_RG::addCheckerboard(double gridSize,
                                                                   double xLength,
                                                                   double yLength,
                                                                   double reflectanceI,
                                                                   CollisionGroupType collisionGroup,
...
void bullet_sim::World_RG::loop(double dt, double realTimeFactor) {
  while (visualizerLoop(dt, realTimeFactor))
    integrate(dt);
}

void bullet_sim::World_RG::integrate(double dt) {
  world_.integrate(dt);
}
...
void bullet_sim::World_RG::visEnd() {
  gui_->end();
...
bool bullet_sim::World_RG::visualizerLoop(double dt, double realTimeFactor) {
  if (!isReady_)
    visStart();
...
void bullet_sim::World_RG::cameraFollowObject(rai_graphics::object::SingleBodyObject *followingObject,
...
void bullet_sim::World_RG::cameraFollowObject(bullet_sim::SingleBodyHandle followingObject,
                                              Eigen::Vector3d relativePosition) {
  cameraFollowObject(followingObject.visual()[0], relativePosition);
}

void bullet_sim::World_RG::setLightPosition(float x, float y, float z) {
  RAIFATAL_IF(!gui_, "RaiSim is running without visualization")
  lightProperty_.pos_light = {x, y, z};
  gui_->setLightProp(lightProperty_);
}

void bullet_sim::World_RG::updateFrame() {
  RAIFATAL_IF(!gui_, "use different constructor for visualization")
...
void bullet_sim::World_RG::processSingleBody(bullet_sim::SingleBodyHandle handle) {
  sbHandles_.push_back(handle);
...
void bullet_sim::World_RG::processGraphicalObject(rai_graphics::object::SingleBodyObject *go, int li) {
  gui_->addObject(go);
  // TODO?
...
void bullet_sim::World_RG::adjustTransparency(rai_graphics::object::SingleBodyObject *ob, bool hidable) {
  if (!hidable) return;
